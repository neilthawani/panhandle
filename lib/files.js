...
module.exports = {
    ...
    copyProjectContents(projectName) {
        ...
        // TODO: Verify contents of sourceDir and destinationDir
    ...
    editBaseTemplate(projectName) {
        ...
        fs.readFile(baseLayoutFilePath, 'utf8', function (error, data) {
            if (error) {
                console.log("Error:", error);
        }

        var result = data.replace(/{{projectName}}/g, projectName);

        fs.writeFile(baseLayoutFilePath, result, 'utf8', function (error) {
            if (error) {
                console.log("Error:", error);
            }
        });
